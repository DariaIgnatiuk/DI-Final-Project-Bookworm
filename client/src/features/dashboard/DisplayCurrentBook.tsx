...
const DisplayCurrentBook = () => {
    ...
    const reading = () => {
        ...
        <div className='bookCardBig'>
            ...
            {(book.authors)?(<h4>{book.authors}</h4>):<></>}
            {(book.date_start)?(<p><span className='bookCardBigInfo'>Started: </span>{book.date_start}</p>):<></>}
            <p><span className='bookCardBigInfo'>Book Type: </span>{book.booktype}</p>
            <p><span className='bookCardBigInfo'>Read </span>{book.pagetype?.toLowerCase()}{(book.pagetype === 'Percentage')? <></>:<>s</>}: {book.reading_progress} / {book.pagecount} 
    ...
    const finished = () => {
        ...
        <div className='bookCardBig'>
            ...
            <div className='titleAndStatusContainer'>
                ...
                {(book.authors)?(<h4>{book.authors}</h4>):<></>}
                {(book.date_start)?(<p><span className='bookCardBigInfo'>Started: </span>{book.date_start}</p>):<></>}
                {(book.date_finish)?( <p><span className='bookCardBigInfo'>Finished: </span> {book.date_finish}</p>):<></>}
                {(book.score)?(<p> <span className='bookCardBigInfo'>My rating:</span> {book.score}</p>):<></>}
                <p><span className='bookCardBigInfo'>Book Type: </span>{book.booktype}</p>
                {(book.categories)?(<p><span className='bookCardBigInfo'>Categories: </span>{book.categories}</p>):<></>}
                {(book.description)?(<p className='description'><span className='bookCardBigInfo'>Description: </span>{book.description}</p>):<></>}
            </div>
